    ...
    while depth < max_depth and (len(list(filter(lambda x:x[1][-1] == end_token, top_k))) < beam_size):
        ...
        for last_decode in filter(lambda x: x[1][-1] != end_token, top_k):
            ...
            next_input = torch.cat((
                ...
                torch.zeros((1,decoder.hidden_size), dtype=torch.float, device=alphabet.device)), dim=1)
...
def beam_search_predictions(decoder, compressed_string, alphabet, beam_size=2, max_depth=None):
    return list(map(lambda x: x[1], beam_search_predictions(decoder, compressed_string, alphabet, beam_size=2, max_depth=None)))
