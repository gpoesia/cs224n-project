...
def beam_search(decoder, compressed_string, alphabet, beam_size=2, max_depth=None):
    if max_depth is None:
        max_depth = 2*len(compressed_string)
    B = 1 #batch size
    C = alphabet.encode_batch([compressed_string])
    C_indices = alphabet.encode_batch_indices([compressed_string])
    encoder_hidden_states, (enc_hn, enc_cn) = decoder.encoder_lstm(C)
    decoder_state = (decoder.h_proj(enc_hn.transpose(0, 1).reshape(B, -1)),
                     decoder.c_proj(enc_cn.transpose(0, 1).reshape(B, -1)))
    finished = torch.zeros(B)
    encoder_hidden_states_proj = decoder.attention_proj(encoder_hidden_states)
    start_token = alphabet.index_to_char(alphabet.START_INDEX)
    end_token = alphabet.index_to_char(alphabet.END_INDEX)
    #top_k : [(score, string, last_hidden_state, last_cell_state, last_attention)]
    top_k = [(0.0,
              start_token,
              *decoder_state,
              torch.zeros((1,decoder.hidden_size), dtype=torch.float, device=alphabet.device))]
    depth = 0
    while depth < max_depth and (len(list(filter(lambda x:x[1][-1] == end_token, top_k))) < beam_size):
        ...
        for last_decode in filter(lambda x: x[1][-1] != end_token, top_k):
            prev_score, prev_string, prev_hidden_state, prev_cell_state, prev_attention = last_decode
            last_char = prev_string[-1]
            next_input = torch.cat((
                alphabet.encode(last_char)[1].unsqueeze(dim=0),
                prev_attention), dim=1)
            decoder_state = (prev_hidden_state, prev_cell_state)
            (decoder_hidden, decoder_cell) = decoder.decoder_lstm(next_input, decoder_state)
            attention_scores = torch.squeeze(torch.bmm(encoder_hidden_states_proj,
                                                       torch.unsqueeze(decoder_hidden, -1)), 2)
            attention_d = F.softmax(attention_scores, dim=1)
            attention_result = torch.squeeze(torch.bmm(torch.unsqueeze(attention_d, 1),
                                             encoder_hidden_states), dim=1)
            U = torch.cat([decoder_hidden, attention_result], dim=1)
            V = decoder.output_proj(U)
            timestep_out = decoder.dropout(torch.tanh(V))
            last_output = F.softmax(decoder.vocab_proj(timestep_out), dim=1)
            predictions, indices = last_output.topk(beam_size, dim=1)
            #import pdb; pdb.set_trace()
            probs = predictions[0] # batch 0
...
def beam_search_predictions(decoder, compressed_string, alphabet, beam_size=2, max_depth=None):
    return list(map(lambda x: x[1][1:-1], beam_search(decoder, compressed_string, alphabet, beam_size=beam_size, max_depth=max_depth)))
